...
- 문자 판독
  - `char.isalnum()` : char가 영문자 혹은 숫자일 경우 True, 아니면 False 반환
  - ` char.isdigit()` : 숫자면 True
- 문자열 바꾸기
...
- get()
  ...
  - key를 이용하여 value를 얻음
  - key가 존재하지 않는 경우 에러메시지를 반환하는게 아니라 **None을 반환한다**는 특징이 있음
...
- `sort()`
  ...
  - 예시2) 2번째 이후의 원소들만 정렬하되 순서가 같으면 1번째 원소 기준으로 정렬하기
    ...
    - 위와 같이 **정렬의 우선순위가 필요하면 튜플로 지정**할 수 있음
...
- 탐색 대상의 시작과 끝 값을 이용하여 **비례식을 구성해 탐색 위치를 결정**하는 방식
...
- 이진 탐색과 똑같이 구현하되 탐색할 인덱스 `mid`를 위 수식을 이용하여 설정하면 됨
- 탈출 조건의 설정
  - 이진 탐색처럼 `if(left > right)`를 이용해서는 안 됨
    - mid값이 left~right범위를 벗어남에 따라 **무한 루프**에 빠질 수 있음
  - `if(arr[left] > target || arr[right] < target)`로 설정해야 함
    - 조금 더 넓은 조건을 설정
...
정렬되어 있는 부분과 그렇지 않은 부분으로 나누어 **정렬 안 된 부분의 데이터를 정렬된 부분에 삽입**하는 것
...
- 기준 데이터를 설정하고 **기준보다 큰 데이터와 작은 데이터의 위치를 바꿈**
  - **기준을 정하는 방식**에 따라 Hoare Partition, Lumoto Partition이 존재
...
1. Radix(기수)의 갯수에 해당하는 길이의 배열을 만든다
   - 10진법이면 길이 10
2. 각 데이터를 순서대로 현재 자릿수의 숫자를 인덱스로 하여 배열에 저장
   - 배열의 원소는 Queue여야 함 : 14, 34면 인덱스 4에 두 숫자가 순서대로 저장되어야 하기 때문
3. 다음 자릿수로 이동하여 배열에서 데이터를 꺼낸 후 2를 반복
...
- 다이나믹 프로그래밍으로는 최적 부분 구조를 가지며 **중복된 하위 문제들**로 풀 수 있는 문제를 푼다
...
귀류법으로 증명 가능 [네이버 블로그](https://m.blog.naver.com/ao9364/221986534307)
...
1. 하나의 정점을 시작으로 MST가 될 때까지 트리를 확장해 나감
...
2. 간선을 **비용순서대로 추가**하는데, **사이클을 발생시킨다면 추가하지 않음**
