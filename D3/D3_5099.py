...
# pop(0)을 이용하여 훨씬 간결하게 짤 수 있음
def oven2():
    oven_list = [[i, c_list[i-1]] for i in range(1, n+1)]
    remain = [[i, c_list[i-1]] for i in range(n+1, m)]
    
    while len(oven_list) > 1:
        num, cheese = oven_list.pop(0)
        cheese = cheese // 2
        if cheese:
            oven_list.append([num, cheese])
        else:
            if remain:
                oven_list.append(remain.pop(0))
