...
- OOP: 프로그램을 객체의 모임으로 파악하는 패러다임
  ...
  - **인터페이스란?**
    - 인터페이스를 **IMPLEMENT**한 클래스들이 동일한 동작을 수행하도록 **강제함**
      ...
      - 정적: 반드시 사용
    - **추상클래스**와는 존재 목적이 다름: 추상 클래스는 상속받아서 기능을 이용하고 확장시키는 것이 목적이고 인터페이스는 함수 구현을 강제하기 위해 사용
...
> 컴퓨터 시스템의 기능, 조직, 구현에 대한 법칙 및 방법을 배움
>
> 크게 명령어 집합 구조(ISA, 기계어), 마이크로 아키텍처(ISA의 구현 방법), 시스템 디자인으로 나뉨
...
- 문제점
  - 전압이 낮으면 트랜지스터 누설 전류 증가
    - 서버 칩의 경우 전체 전력의 40%가 누설 전류로 소모
...
- 장단점
  ...
  - 잦은 변경 시 성능저하(DML이 자주 일어나는 경우)
    - 삽입 시 Index split이 일어나는 동안 DML이 블로킹 됨
    - 삭제 시 인덱스와 테이블의 데이터 수가 달라질 수 있음
    - 인덱스는 업데이트 불가: 업데이트시 인덱스 삭제 및 삽입을 해야 되므로 작업 2배
- 종류
...
- DBMS의 구조
  - Query Processor
    - 입출력 단위는 고정 길이의 페이지 단위
  - Storage System
    - 일부분을 메인 메모리에 저장하고, 대부분을 disk에 저장
    - Buffer Manager: 메인 메모리에 어떤 페이지를 저장할 것인가?
  - UNDO
    - 정상적으로 종료되지 않은 페이지들을 원상 복구하는 것
    - 수정된 페이지를 언제 디스크에 쓰는지에 따라 2가지로 분류함
      - steal 적당히 미리 기록함: 메모리 버퍼가 많이 필요하지 않고, UNDO logging이 필요
      - no-steal 트랜잭션이 완료된 이후에 씀
  - REDO
    - 이미 commit한 트랜잭션의 수정을 재반영하는 복구 작업
    - 트랜잭션이 종료되는 시점에 해당 트랜잭션이 수정한 페이지를 디스크에 쓸 것인지 말 것인지
      - force: 수정한 모든 페이지를 transaction commit 시점에 반영, redo 필요없음
      - no-force: 커밋 시점에 반영하지 않음, 트랜잭션이 db에 반영되지 않을 수 있기에 redo 필요(대부분)
- Locking
  - 다른 트랜잭션이 관여하지 못하도록 막는 것
    - 동시성과 무결성에 trade-off 존재함
  - Isolation level
    - Read Uncommited (level 0)
      - 처리중이거나 아직 커밋되지 않은 데이터를 읽는걸 허용, 연산은 불허
...
- 구성
  ...
  - 주소 체계: IP주소 방식, DNS주소 방식
    - **DNS란?**
    - Domain Name System: IP가 아니라 이름으로 연결하기 위한 서버
      1. 호스트가 DNS 서버에게 IP를 이름으로 바꿔달라는 요청을 보냄
      2. 클라이언트가 로컬에서(호스트 파일) 도메인 이름을 찾음
      3. 없으면 DNS에 해당 이름의 IP 주소를 요청, 응답을 받음
      4. DNS서버의 주소는 인터넷에 연결하는 순간 **DHCP**가 세팅해줌
      5. IP주소를 이용해서 접속하여 통신
    - **DHCP**란?
...
- 기본 용어
  ...
  - 프로토콜: 데이터 교환을 원활히 하기 위한 통신 규약
    ...
    - 타이밍: 속도 조절과 순서 관리
  - 웹: 인터넷의 일부
    - 클라이언트-서버 간에 HTTP 프로토콜을 사용하여 HTML을 주고받는 시스템
  - 호스팅: 대형 서버의 기능을 빌려쓰는 것
    - 웹호스팅: 하나의 서버 장비를 여러 명이 공유하여 사용
    - 서버호스팅: 한명의 고객이 하나의 서버장비를 임대
      - 고정적으로 트래픽 양이 많은 사이트에 적합
    - 클라우드호스팅: 가상서버를 임대하고 이용한만큼 지불
...
- 네트워크
  - 단위: 패킷, 장치는 **라우터**
    - 라우터: 컴퓨터간 연결 수를 줄이기 위한 근거리 허브 역할을 함
    - ISP: 원거리의 라우터를 연결함
  - 라우팅 프로토콜을 사용하여 최적의 경로 선택, IP주소를 지정
- 전송: TCP, UDP
  - 단위: 세그먼트 - 데이터를 세그먼트에 감싸서 네트워크층으로 넘김
  - 신뢰성 있는 데이터를 주고 받을 수 있게 함
  - 상위 계층들이 데이터 전달의 유효성이나 효율성을 생각하지 않도록 함: **어떻게?**
  - **포트**를 열어두고 프로그램들이 전송할 수 있도록 제공해 줌
    - 포트: TCP나 UDP에서 어플리케이션이 상호 구분을 하기 위해서 사용하는 번호
- 세션
  - 단위: 데이터, 장치는 L5 스위치
...
- 흐름제어/혼잡제어
  - 흐름제어: **송신측과 수신측**의 데이터 처리속도 차이 해결
    - receiver가 sender에게 현재 자신(receive buffer)의 상태를 feedback하여 패킷을 지나치게 많이 받지 않도록 함
