...
> 아키텍처가 하드웨어를 배운다면, OS는 하드웨어와 소프트웨어를 **효율적으로** 운영하기 위한 이들의 동작원리를 배운다
...
> 32bit OS에서는 0부터 2^32-1까지 영역이 나누어진 것이 하나의 세그먼트로 Stack, Heap, BSS, Data, Text로 이루어짐

- Stack
  - 함수에 대한 정보를 포함: 지역변수 등 
  - 함수를 호출할 때마다 스택 영역에 쌓이게 되면서 **buffer overflow**같은 공격들이 실행될 수 있음
  - 버퍼, 변수, RBP(Base Point), RET(Return Address)로 구성됨
    - RBP: 스택이 쌓이기 시작하는 지점
    - RET: 함수가 끝난 후 돌아가는 (메모리의) 위치
    - main내에서 sum을 불러오면 RET-RBP-변수c-버퍼-변수x-변수y-RET-RBP-버퍼 순으로 쌓이게 됨
      - 함수를 불러올 때는 해당 함수의 파라미터가 RET아래에 들어감 
- Heap
  - 동적 할당된 변수들이 저장됨
- BSS
  - 프로그램에서 사용될 변수들: 초기화되지 않은 것
- Data
  - 초기화 된 변수들
- Text
  - 소스 코드
...
- 프로세스
  - **실행중인** 프로그램: OS로부터 시스템 자원을 할당받는 작업의 단위
    - 프로그램은 실행되기 전 상태의 명령어의 집합
  - 스택(임시 자료), 힙(동적 할당 메모리), 데이터(전역 변수), 텍스트로 구성 
    - **동적 할당 메모리란??**
    - 입력되는 데이터에 맞게 기억공간을 확보하는 것!
    - C에서는 `malloc()`함수, 자바는 `new()`함수로 힙에 할당하게 됨
  - - 
  - **Multi Process**
    - 한 작업을 여러 프로세스가 처리
...
- UDP: TCP에서 신뢰성 기능을 뺀 것(스트리밍 하려고 만든 것임)
  - 실시간성을 보장해 줌
- IP
  - 각 장치를 나타내는 주소: 호스트나 라우터 장비의 인터페이스에 할당
